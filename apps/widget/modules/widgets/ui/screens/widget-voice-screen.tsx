...
export const WidgetVoiceScreen = () => {
  ...
  const {isConnected, isSpeaking, transcriptMessages, startCall, endCall, isConnecting} = useVapi()
    ...
    <WidgetHeader>
        ...
        <div className="border-t bg-background p-4">
          <div className="flex flex-col items-center gap-y-4">
            {isConnected && (
            <div className="flex items-center gap-x-2">
              <div className={cn("size-3 rounded-full animate-pulse", isSpeaking ? "bg-green-500" : "bg-red-500")} 
              />
              <span className="text-xs text-muted-foreground">
                {isSpeaking ? "Assistant Speaking..." : "Assistant Listening..."}</span>
            </div>            
            )}
